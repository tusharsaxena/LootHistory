...
| **luacheck** | any recent (developed against 1.2.0) | The lint gate, `luacheck .`, must report 0 warnings / 0 errors before every commit (`docs/testing.md:187`). Config is `.luacheckrc`. | `luacheck --version` |
...
| **bash** | **4.0+ — a hard requirement** | `tests/_kit/run-automated-tests.sh` is `#!/usr/bin/env bash` and declares associative arrays (`declare -A ST DUR NOTE`, `:186`), which `sh`/`dash` cannot parse; `:109` additionally prefers bash 5.0's `EPOCHREALTIME` for millisecond timing and falls back cleanly below it. Only the automated-test **bundle** needs it — `lua tests/run.lua` and `luacheck .` do not. | `bash --version` |
