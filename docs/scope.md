...
- **Currency capture** (Valorstones, crests, etc.) as `Type=Currency` history rows — attributed to the same sources as items, obeying the per-source mute list and a `Record currency` master toggle, but exempt from the quality/quest gates. Each currency record carries its own `C_CurrencyInfo` quality tier: the History table colors the currency **Name** cell and fills the **Quality** column the same way it does for items, and hovering a currency row shows the in-game **currency tooltip** (`GameTooltip:SetCurrencyByID`) — still excluded from item-centric quality aggregates. Surfaced in the History table, a dedicated Insights currency block, and CSV export. Currencies can be **blacklisted** (a separate `global.currencyBlacklist` id-set, point-in-time, same as the item blacklist) — there is no currency whitelist. (This reverses the earlier "currencies out of scope" decision — ratified 2026-07-21.)
...
- **AI / LLM export.** Rendering the loot data into an AI-generated report is out of scope — the addon exports to **CSV only**. (An earlier Export-to-AI feature was removed 2026-07-26.) The `Database:Export()` seam that CSV builds on remains.
...
Revisiting the single-slot TTL and value/upgrade addon interop are tracked there. Shipped from the tracker: shared filters across History + Insights (#13), the item-id blacklist/whitelist (#14), and tab-aware Export with an Insights CSV (#15).
