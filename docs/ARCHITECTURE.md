...
published (`NS.HOLD_PERF`) and the latch honours it, because the invariant is the library's rather
...
`GetQuestReward` — and that one funnel gates its body and returns. It is not licence to gate
