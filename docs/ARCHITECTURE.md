...
- **`NS.db.global.settings.window`**, the window geometry `{ point, x, y, w, h }`. The local
  ...
  **Reset position** button (`onResetPosition`, `settings/Schema.lua:88`) and from **Reset all
  settings** (`Sl:ResetEverything`, `settings/Slash.lua:132`).
- **`NS.db.global.savedView`**, the remembered view. `B:SaveView` (`:667`) stores the whole
  ...
  `Sl:ResetEverything` (`settings/Slash.lua:131`). The load pass reshapes it (migrations `to = 6`
