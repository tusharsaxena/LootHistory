...
- **`NS.db.global.settings.window`**, the window geometry `{ point, x, y, w, h }`. The local
  ...
  **Reset position** button (`onResetPosition`, `settings/Schema.lua:109`) and from **Reset all
  settings** (`Sl:ResetEverything`, `settings/Slash.lua:198`).
- **`NS.db.global.savedView`**, the remembered view. `B:SaveView` (`:663`) stores the whole
  ...
  `Sl:ResetEverything` (`settings/Slash.lua:198`). The load pass reshapes it (migrations `to = 6`
...
> `Database:PruneOld` (behind the prune confirm) directly, and `minimap.hide` has no `onChange` at all: it is the one stored
