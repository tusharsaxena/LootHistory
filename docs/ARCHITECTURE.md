...
| `modules/Filters.lua` | `NS.Filters`: the blacklist/whitelist item-id lists — `Add`/`Remove` (copy-on-write, mutually exclusive), `Blacklist`/`Whitelist` (the live id sets), `SortedIDs` — **plus the currency blacklist** (`AddCurrencyBlacklist`/`RemoveCurrencyBlacklist`, `CurrencyBlacklist`; blacklist-only, keyed by `currencyID`). Parsing and labelling an entry is the Filters tab's LibKa0s IdList's job, not this module's. On change: a direct `Collector:RefreshUpvalues()` re-cache + `Database:FireHistoryChanged()`. Data-only; loads before Collector; no `Enable`. |
...
- **`NS.db.global.settings.window`**, the window geometry `{ point, x, y, w, h }`. The local
  `SaveWindow` (`:101`) writes it when a title-bar drag stops (`:968`) and when the resize grip is
  released (`:1056`). `B:ResetWindow` (`:685`) writes `{}`, reached from the Master controls
  **Reset position** button (`onResetPosition`, `settings/Schema.lua:95`) and from **Reset all
  settings** (`Sl:ResetEverything`, `settings/Slash.lua:158`).
- **`NS.db.global.savedView`**, the remembered view. `B:SaveView` (`:667`) stores the whole
  `B:CaptureView()` when the player clicks the filter bar's **Save** (`:758`). `B:ResetView`
  (`:676`) clears it to `nil`, reached from the filter bar's **Reset** (`:755`) and from
  `Sl:ResetEverything` (`settings/Slash.lua:157`). The load pass reshapes it (migrations `to = 6`
  and `to = 8` in `core/Database.lua`).
- **`NS.db.global.minimap`**, apart from its `hide` row. `B:SetupMinimap` (`:1202`) hands the table
...
| `compat-layer.md` | Present | `core/Compat.lua` is 411 lines of addon-specific shimming beyond LibKa0s |
