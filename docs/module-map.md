...
└── modules/
    ...
    ├── Browser.lua       — standalone §6A window shell: frame, skin, tabs, filter bar,
...
4. `core/Namespace.lua` — sets `NS.name`, `NS.version` (`"1.1.0"`, `Namespace.lua:6`), and `NS.PREFIX` (`|cff33ff99[LH]|r`). Modules still self-publish idempotently; nothing else wired here.
...
10. `locales/enUS.lua` — `NS.L` with a metatable that returns the key itself, so English strings work untranslated and missing keys never error. v1.0.0 ships English-only: no user-facing string routes through `NS.L` yet (an accepted scope decision, `enUS.lua:7-11`); the seam is kept for a future localization pass.
11. `settings/Schema.lua` — `NS.Schema` (alias `S`): one row per setting (`S.Schema`, `Schema.lua:10-64`) driving AceDB defaults, panel widgets, and the slash CLI. `S:Set(path, value)` is the single user-setting write seam — validate → deep-copy → write to `NS.db.global` → `onChange` (`Schema.lua:107-117`); the deep-copy stops a reset from aliasing the DB to a shared default table. Also owns `NS.COMMANDS`, the slash dispatch/help table (`Schema.lua:140-171`). `debug` is intentionally not a Schema row.
12. `settings/Slash.lua` — `NS.Slash`. Registers `/lh` and `/loothistory` via AceConsole (`Slash.lua:39-42`), dispatches the verb against `NS.COMMANDS`, and generates the help index from it (bare `/lh` prints help — Ka0s §7.4). Provides the schema-driven `CliGet` / `CliSet` / `CliList` / `CliReset` / `CliResetAll`, the `ResetEverything` full wipe, and registers the purge / reset-all `StaticPopupDialogs` confirm dialogs at file-load (`Slash.lua:10-30`). See [slash-dispatch.md](slash-dispatch.md).
13. `settings/Panel.lua` — `NS.Panel`. Registers the parent `Settings.RegisterCanvasLayoutCategory` (a logo + tagline + slash-command landing page) plus a `General` `RegisterCanvasLayoutSubcategory` holding the real settings. Renders `NS.Schema.Schema` into a lazy two-column AceGUI grid (checkbox/dropdown/slider/multi-check makers), a live History stats + Purge section, and the always-shown-scrollbar patch (`Panel.lua:108-160`, Ka0s §6.10). Writes route through `NS.Schema:Set`; `P:Open` is combat-gated. See [settings-panel.md](settings-panel.md).
