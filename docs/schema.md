...
that row's `onChange` raises (`S:OnRetentionChanged`, `settings/Schema.lua:708`). `Purge` (`core/Database.lua:745`) empties it from the purge confirm
(`settings/Slash.lua:13`) that `/lh purge` and **Purge history…** open, or directly with no
`StaticPopup_Show` (`settings/Schema.lua:810`, `settings/Panel.lua:113`). `Delete`
