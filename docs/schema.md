...
- `settings.excludedSources` is stored as the set of **muted** sources; the panel renders it inverted ("Record data from"), so a checked box means "record this source" (`settings/Schema.lua:275`).
...
that row's `onChange` raises (`S:OnRetentionChanged`, `settings/Schema.lua:736`). `Purge` (`core/Database.lua:745`) empties it from the purge confirm
(`settings/Slash.lua:13`) that `/lh purge` and **Purge history…** open, or directly with no
`StaticPopup_Show` (`settings/Schema.lua:939`, `settings/Panel.lua:113`). `Delete`
...
`settings/Slash.lua:202`) replaces all of it wholesale. Purge, delete and that reset each log one
