...
`minimap` is not on that list because LibDBIcon owns it: `Browser` hands the table to `DBIcon:Register` (`modules/Browser.lua:1237`) and the library keeps the button's state in it, storing `minimapPos` when the button is dragged. Only its `hide` leaf is a schema row. `B:SetupMinimap` no longer seeds the table when it is missing (#30): replacing the whole table would be a write over the `minimap.hide` row, and the AceDB default (`defaults/Global.lua:63`) already supplies it. **Reset all settings** replaces the table, so it hands LibDBIcon the new one (`B:RefreshMinimap`); otherwise a drag before `/reload` would store `minimapPos` in the old table and the position would be lost.
...
`settings/Slash.lua:108`) replaces all of it wholesale. Purge, delete and that reset each log one
