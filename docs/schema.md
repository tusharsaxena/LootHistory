...
- `history` is a **dense array** — `Database:Delete`/`PruneOld` rebuild-and-swap rather than leaving holes (`core/Database.lua:718`, `:801`). Each record's field shape is documented below.
...
- `Database:PruneOld()` (`core/Database.lua:801`) — retention cleanup; drops records older than `settings.retentionDays` (`0` == keep Always), gated once per session.
...
| `minimap.hide` | General ▸ Master controls | CheckBox | `true` *(the row's sense)* | The LibDBIcon button's visibility, composed from `minimapPath` (launcher-§3). **The row says SHOWN and the stored key says HIDDEN**, so its `get`/`set` invert at `Schema:Set` — the only **stored** row with accessors of its own — and the `set` calls `NS.Launcher:SetShown`, which moves the button immediately rather than at the next reload. `defaults/Global.lua` ships `hide = false`, which is the same fact in the other sense. The path carries no `settings.` prefix because LibDBIcon's table lives in the global store, outside the block's. It replaced the Interface tab's **Hide minimap button**. **Exempt from every bulk reset** (launcher-§3, standard v2.54.0): `NS.Schema.RESET_EXEMPT` names the path once, `Schema:ApplyDefault` skips it while a bulk bracket is open, and `wipeGlobal` carries the stored value across *Reset all settings*. A single `/lh reset minimap.hide` still resets it — that is the player naming the row. |
...
Its one owner, `NS.Database` (`core/Database.lua`), holds every writer. `Add` (`core/Database.lua:287`)
appends each kept loot or currency line (`modules/Collector.lua:140`, `:205`). `PruneOld`
(`core/Database.lua:801`) drops rows past `settings.retentionDays` once per session after
`PLAYER_ENTERING_WORLD` (`core/LootHistory.lua:75`) and when the player accepts the prune confirm
that row's `onChange` raises (`S:OnRetentionChanged`, `settings/Schema.lua:694`). `Purge` (`core/Database.lua:737`) empties it from the purge confirm
...
`bound` (`core/Database.lua:219`) from two deferrals after login (`core/LootHistory.lua:83`, `:87`)
and each window open (`modules/Browser.lua:1066`). **Reset all settings** (`Sl:ResetEverything`,
`settings/Slash.lua:193`) replaces all of it wholesale. Purge, delete and that reset each log one
...
(`core/Database.lua:245`) advances `boundRepairAttempts` and clears both once nothing is pending or
...
Three reset surfaces write these tables; each reaches a deliberately different scope so the state above is never silently missed (audit 2026-07-17):
...
Each logs its settings reset once (`debug-logging-§10`). The non-destructive reset logs `[Set] reset all: N rows`, N the rows whose stored value changed, with ` (stopped by an error)` appended when a row raised part-way (the line is still logged once, the mute released and the error re-raised). The destructive one is a wholesale replacement, not a walk through `Schema:Set`, and with no profile it is this addon's reset-profile equivalent (`options-ui-§12`): it logs one `[Set] reset account-wide settings to defaults (N rows)`, N the stored rows that were not at their default just before the wipe (the session-only console and test-mode rows are not counted), the same line BankLedger logs. The wipe never reaches a session-only row, so it switches test mode off itself (`BrowserTable:SetTestMode(false)`, no `[Set]` line), because options-ui-§15 says Reset all settings ends it. Its `[Data] reset-all removed N rows` line stays beside it, as the separate `debug-logging-§8` trace of the history it discards. The single reset logs `[Set] <path> = <value>`.

**The global reset is wholesale, and no longer a composition.** (It was labeled "Reset Everything" on the Maintenance tab until options-ui-§15 gave it its canonical name and place.) It used to be three enumerations —
...
`Database:PruneOld` (`core/Database.lua:801`) enforces `settings.retentionDays`: it drops every record older than `now - retentionDays × 86400`, rebuild-and-swap, and fires `Ka0s_LootHistory_HistoryChanged`. `retentionDays == 0` means "keep Always" and returns early. It runs once per session after login, and after a retention change — but a change never prunes on its own say-so.

The row's `onChange` is `S:OnRetentionChanged` (`settings/Schema.lua:694`). It asks `Database:CountOlderThan(days)` (`core/Database.lua:788`, an allocation-free count, `0` for Always) how many records the new value would drop:

- **None** — the value becomes the confirmed retention and nothing else happens.
- **Some, in-game** — it raises `KA0S_LOOTHISTORY_PRUNE` (`settings/Slash.lua:23`), naming the new retention and the count. **Yes** runs `PruneOld`. **No** writes the last *confirmed* retention back through `Schema:Set` and prints `retention kept at <label>; no records were deleted.` — so the stored value, which the login prune reads, never holds a retention the player refused, and the next login deletes nothing either (`S:ConfirmRetention`, `settings/Schema.lua:708`).
...
`global.currencyBlacklist` (`{ [currencyID]=true }`) is a third id set in the same `NS.Filters` registry, alongside `blacklist`/
