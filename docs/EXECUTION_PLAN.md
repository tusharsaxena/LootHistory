...
- [ ] **Tune attribution (M1 follow-up).** In-game smoke (2026-07-11) confirmed KILL attribution + quality gate, but surfaced two refinements:
  ...
  2. **Source-name resolution.** `LOOT_OPENED` currently stamps `sourceName = nil` for KILL/CONTAINER (source *type* + detail are correct). Resolve the mob/container display name from the loot-slot GUID/tooltip so records carry a human `sourceName`.
