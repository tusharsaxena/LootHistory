...
### `Database@268-356 (real name: Database:QueryList)` — CCN 58 → target 9
...
**Fix.** Replace the six hand-written scalar-or-set clauses with a module-level constant descriptor table and a shared matcher. This is a HOT path (every filter change re-runs it across the whole history, and BrowserTable renders off the result), so the descriptor table must be module-level and the record loop must allocate nothing per record.
1) `local SCALAR_OR_SET = { { f = "source" }, { f = "char" }, { f = "itemType" }, { f = "itemSubType" }, { f = "zone", default = "" } }` — file-local constant, built once at load.
2) `local function matchQuality(filter, r)` — the number/table/ignore-anything-else split. CCN ~5.
3) `local function matchScalarOrSet(filter, r)` — `for i = 1, #SCALAR_OR_SET do local s = SCALAR_OR_SET[i]; local want = filter[s.f]; if want ~= nil then local rv = r[s.f]; if s.default ~= nil then rv = rv or s.default end; if type(want) == "table" then if not want[rv] then return false end elseif rv ~= want then return false end end end; return true`. CCN ~9. Numeric `for` over a constant array — no allocation, no closure.
4) `local function matchRange(r, boundSet, from, to, text)` — the bound set test (`r.bound or "NONE"`), the from/to ts window, and the lowered-substring test. CCN ~9. `text` is lowered ONCE by the caller and passed in, exactly as today.
5) `QueryList` itself: `filter = filter or {}`, hoist `boundSet`/`from`/`to`/`text`, then `for _, r in ipairs(records) do if matchQuality(filter, r) and matchScalarOrSet(filter, r) and matchRange(r, boundSet, from, to, text) then out[#out+1] = r end end`. CCN ~8.
Note the `char` descriptor has no default, matching today (`r.char ~= char`, no `or ""`), and only `zone` gets the `""` default. `bound` stays out of SCALAR_OR_SET because it is set-only (a non-table bound filter must be ignored, per the existing test).

**Must not change.** A non-number, non-table `quality` (e.g. the stray "all" sentinel) must be IGNORED, not crash and not filter — same for a non-table `bound`. `zone` matches `r.zone or ""` so a record with no zone lands in the empty-string bucket; `bound` matches `r.bound or "NONE"`. from/to are inclusive against `r.ts or 0`. `text` is lowered once per call, not per record. The returned array holds the SAME record references (not copies) in input order — Export and the table both rely on that. Hot path: nothing may be allocated inside the record loop.
