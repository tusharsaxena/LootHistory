...
1) `local function matchesField(have, want, isSet)` — one scalar-or-set clause, `isSet` already computed by the caller. CCN 2.
2) `local function matchQuality(r, qSet, qExact)` — the set/number/ignore-anything-else split, both forms hoisted. CCN 4.
3) `local function matchScalarOrSet(r, src, srcIsSet, chr, chrIsSet, itype, itypeIsSet, isub, isubIsSet, zone, zoneIsSet)` — the five clauses written out, each guarded by its hoisted filter value so an unfiltered clause costs one local test and no call. CCN 12.
