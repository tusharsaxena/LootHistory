...
| `test_launcher.lua` | the **`LibKa0s-Launcher-1.0`** seam (`core/LauncherSetup.lua`) — the minimap button and the broker plugin as **one** object registered twice (launcher-§1): one LDB object of `type = "launcher"` handed straight to LibDBIcon under the addon's **folder** name, wearing the addon's own logo, with `Register` proved idempotent; **rung (a)** (launcher-§2) — left-click reaching `NS.Browser:Toggle`, the switch `/lh toggle` calls, and pointedly **not** the settings panel, which right-click always opens; the **Minimap button** row driven through `NS.Schema:Set` inverting onto LibDBIcon's own `hide` key and moving the real button (launcher-§3); the seed over `db.global.minimap` still absent; *Reset all settings* re-pointing LibDBIcon at the store's new table; and the **degraded** host with neither broker library, where nothing raises, `Register` reports itself absent and the store is still the truth. The **icon file** is checked as bytes — TGA image type 2, 32 bpp, 128×128 (layout-§4) — because a wrong format there draws nothing and raises nothing, and the TOC's `## IconTexture` is compared against the LDB object's own `icon`. The **status tooltip** (Launcher minor 3) is read through the object's `OnTooltipShow` as exact lines, enabled and disabled — title with the TOC version, `Enabled`, `Locked`, `Test mode`, the record count, `Left-click: Toggle History window` or `disabled — /lh enable`, `Right-click: Open settings`, nothing twice — with the lock and test mode flipped between shows; the descriptor case re-runs `core/LauncherSetup.lua` into a scratch namespace and pins `version`, `isLocked`, `isTestMode`, `leftClickLabel` (through `NS.L`) and an `onTooltipShow` that draws the count alone |
