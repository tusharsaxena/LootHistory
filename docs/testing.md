...
- **AceEvent's EVENT half.** The kit models the `(message, target)` bus and nothing else, because that is the half every LibKa0s consumer uses — but real `AceEvent-3.0`'s `Embed` stamps `RegisterEvent`/`UnregisterEvent` alongside it, and `modules/Browser.lua` registers the two combat transitions the General visibility setting is about on its **own** embedded target. Without this half that registration is a call on a nil field. Keyed the same way, on one shared registry, and exposed through `mocks.__fireAceEvent(event, …)` so a suite can drive a combat transition rather than reaching into the module. The addon object keeps the kit's own recording `RegisterEvent`, so `NS.bus:RegisterEvent` still lands in `__events` where every other suite has seen it.
...
Twenty-six files, loaded in this order (see **[test-cases.md](test-cases.md)** for the full per-case
