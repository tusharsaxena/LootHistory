...
- **Tab art that measures differently when selected, and a recorded `SetPoint`.** options-ui-§13 makes a wrapped strip's geometry selection-invariant and then names the harness the case needs: *"A harness that answers one height for every atlas cannot fail this — the harness MUST answer a different height for the selected-state art, or the case is green against nothing."* The kit's stub is exactly that harness — `SetAtlas` no-ops through the catch-all metatable — so the two atlas families are given different heights here (`Options_Tab_*` 30, `Options_Tab_Active_*` 34, both under the library's `TAB_H` clamp of 37), applied only where nothing has *set* a height so a tab button keeps its real one. `SetPoint` is recorded on the same layer, because where a tab lands reaches the frame through nothing else and *"every row's y offset is identical for every value of the selection"* is otherwise a sentence no assertion can be written against.

- **`GetStringWidth` and `InlineGroup:SetTitle`.** The AH price table positions each row's info icon at `gutter + ACOL.module + GetStringWidth() + 6`, and the kit's blanket "any PascalCase method returns the frame" hands that a table; the inverted set picker draws into an AceGUI `InlineGroup`, whose `SetTitle` has no LibKa0s consumer and so is not modeled.

AceEvent's **event** half is no longer a local extension. Since kit revision 16 (LibKa0s v1.30.0) the kit's `Embed` stamps `RegisterEvent` / `UnregisterEvent` / `UnregisterAllEvents` on every target, validates them as CallbackHandler does, and records per target on `t.__events`, the same functions and the same field the `NewAddon` target has always had. `modules/Browser.lua` registers the two combat transitions the General visibility setting is about on its **own** target, `B.__ev`, and `tests/test_browser.lua` drives them by firing the recorded handler, `B.__ev.__events[event](event)`. The shared-registry shim and its `mocks.__fireAceEvent` helper that did this before are gone: kit 16's `Embed` switched them off.

Neither is the **message** half. Up to kit revision 16 the kit's `NewAddon` ignored its mixin list, so `tests/wow_mock.lua` wrapped it to embed AceEvent by hand, because `NS.bus` *is* the addon object. From revision 17 (LibKa0s v1.31.0) the kit embeds exactly the listed libraries, so `NewAddon(NS, name, "AceEvent-3.0", "AceTimer-3.0", "AceConsole-3.0")` gets the message half, AceTimer and AceConsole from the kit. The wrapper is gone, and `tests/test_harness.lua` pins that `NS.bus` is the addon object and carries the listed mixins.
...
Thirty-eight suites (see **[test-cases.md](test-cases.md)** for the full per-case inventory and the
authoritative count): thirty-four files of this repo's own under `tests/`, and four the kit ships under
...
| `test_compat.lua` | `NS.Compat` shims — GUID decode, item/map info, degraded fallbacks; `GetSpellName` as **`LibKa0s-Compat-1.0`**'s member: each rung and its one-value arity, the nil-id guard, the nil/`""` fall-through, and the degraded reader stub answering nil |
| `test_attribution.lua` | source-resolution engine — context stamp/consume, TTL, confidence, plus an integration case over `Attribution:Enable` asserting the nine bus events, the player-only `RegisterUnitEvent` cast frame, the five read-side hooks and the re-entry latch |
| `test_filters.lua` | `NS.Filters` blacklist/whitelist id lists — add/remove (mutually exclusive, copy-on-write), `Blacklist`/`Whitelist` set contents, `SortedIDs`, the currency blacklist, and that the add-box parsers IdList replaced are gone |
...
| `test_slash_degraded.lua` | `settings/Slash.lua`'s library-less branch, on a real degraded load (`tests/degraded_env.lua`): the help list rendered by subtraction (`config`, `set` and the schema CLI omitted; `enable`/`disable` offered), the refusal line's template pinned byte for byte against `LibKa0s-Slash-1.0`'s `DISABLED_LINE_FORMAT` (`Kit.assertLibraryConstant`) and rendered with `/lh enable` as the library renders it, `/lh disable`/`enable` writing `settings.enabled` through the seam's `writeThrough` path and moving the latch with one ack line each, any other `set` still answering unavailable, `resetall`'s counted filters line, and both diagnostics forms answering with the library-absent line alone while the help leaves `diagnostics` out |
| `test_schema.lua` | `NS.Schema` rows — `Set` validation + write-through (deep-copied, never aliased), `Get`/`Default`, session-only rows (`state.debugConsole`) never touching `db.global`; plus the schema's own shape: unique paths, defaults matching both their declared type and `defaults/Global.lua`, dropdown defaults being selectable, slider defaults inside their range, every setting round-tripping, the `NS.COMMANDS` table, and the **count-claim gate**: every row-count and per-tab breakdown printed in `ARCHITECTURE.md`, `settings-panel.md` and `module-map.md` is parsed back out of the file and compared against the live schema, in both of `module-map.md`'s two homes; and the seam as **`LibKa0s-Schema-1.0`**'s: return arity and this addon's refusal words, log-then-react order, the host names delegating to `NS.SchemaRuntime`, the pre-db refusal, the read-back bulk tally, `Register`'s shape checks, every degraded read and write a player can reach through the host stub; and the **Keep history for** prune confirm — a shorter value raises `KA0S_LOOTHISTORY_PRUNE` with the count and deletes nothing yet, Yes prunes, No restores the previous value with one chat line, no `StaticPopup_Show` prunes at once, and a value that deletes nothing asks nothing |
| `test_schema_stub.lua` | the host Schema degradation stub's **writers**, on a real degraded load (`tests/degraded_env.lua`) against probe rows: `SetMany`'s all-or-nothing batch (Schema minor 2) — an invalid or unknown entry refuses with its index and stores nothing, a valid batch stores every entry before any `onChange` runs, and `opts.act` brackets it |
...
Seventeen of the thirty-eight exist because of the LibKa0s adoption:

| Suite | Covers |
|-------|--------|
| `test_disabled.lua` | **`slash-commands-§7`'s mandated conformance suite** — the disabled state is TOTAL. Baseline the enabled addon's registration set, disable it **through the single write seam** (never by calling a teardown function, because the test has to exercise the route the checkbox and the verb take), and then assert on the **registration set** through the kit's recording mock: nothing this addon registered survives, no game event of any kind is still registered, every deferral it armed is canceled, every frame it drew is down and the show ladder keeps it down, and firing every event it used to watch writes nothing, prints nothing and draws nothing. Then the surface that does NOT change: every reserved verb still answers, both diagnostics forms write a whole report from the stand-down itself (its capture section saying so, the registration set untouched), the bare `/lh` opens the panel, each feature verb refuses on exactly one line and reaches no write seam, and the addon's restated live set agrees with `lib.LIVE_VERBS`. Then the launcher — a left click that opens the panel and prints nothing, and an options menu whose Enabled entry stays live while Locked, Test mode and Show window are grayed and, force-clicked, reach no handler and write no SavedVariables. Then restoration from **current** state, and the latch: releasing one hold must not resurrect an addon the other is still holding down, in both orders. **Not one case asserts that a handler returned early** — an early return is exactly what a draw gate does, so a suite written against one certifies the thing it exists to catch. Confirmed red against the draw gate by putting it back: steps 3, 4, 6 and 10 all redden. Its bring-up first closes any settings page an earlier suite left on screen (firing the `OnHide` the kit mock's `Hide` does not), because since LibKa0s v1.46.1 the library's combat lock holds `PLAYER_REGEN_*` for exactly as long as one of its pages is shown — the library's, not a survivor of this addon's stand-down |
| `test_panel.lua` | the settings panel, which had **no suite at all** before the `LibKa0s-Options-1.0` adoption. Registration is now asserted as an **exact set** — one subcategory, because a leftover registration is a page nobody draws and a presence check could never see one — plus its idempotence, the deferred first-`OnShow` body render, one widget per non-skipped row at the 50/50 width, `CheckBox`/`Dropdown`/`Slider` dispatch, both enum shapes populated in declared order (the array form and the composed key-map + `sorting` form), slider bounds, the **six-tab strip** in order opening on `Master controls`, the tab list derived from **both** the schema and `GENERAL_TABS` and compared, the canonical Master controls rows and their closing button pair driven through the widgets a user clicks, every tab's partition asserted in both directions (its own rows present, another tab's absent), **no** section heading under a strip but the one mixed tab's **subsection** headings present — AH Price's *Pricing* / *Price sources* (Interface's *Window* / *Minimap* went when the minimap row moved to Master controls), stated as an expected list for all six tabs so a heading that appears where none belongs is as red as one that vanishes — the **wrap-invariance** case options-ui-§13 makes a MUST (a strip forced to two rows reserves the same band and anchors every tab at the same y for all six selections, against a harness that answers a taller height for the selected-state art), no `subgroup` repeating its own tab's name, the write paths reaching `NS.Schema:Set`, an external write mirrored back by `Refresh`, the inverted muted-source picker in both directions, the page-wide Defaults handler reaching both stores with no schema row (the id-list registry and the cascade) and **not** moving the window, the Filters tab's secondary strip and its off-screen dirty repaint, its three `O.IdList`s (an add by id, link or name reaching the list's own `NS.Filters` writer with the `[id] = true` shape kept, the X on the left of each entry (removeStyle = "icon", LibKa0s v1.44.0) calling it, one repaint per add, a late item load naming its entry, suggestions and name lookup drawn from the lists and then the loot history newest first, a name several ranks share refused on Enter whether the ranks come from the history or the bags, and each add box's tooltip ending on the same hint its refusal ends with), the AH tab's pooled slots surviving a second visit unchanged, the host parked off the page while another tab is on screen, the reorder controller's boundary / handles / boxes, a drag landing four positions away in one write, the controller canceled at the TOP of the render (asserted as source order as well as as state), the landing page's command rows matching `lib.FormatRow` byte for byte, and the combat refusal |
| `test_panel_filters.lua` | the Filters tab, peeled out of `test_panel.lua` (layout-§1) — its secondary sub-tab strip, its off-screen dirty repaint, and its three `O.IdList`s: an add by id, link or name reaching the list's own `NS.Filters` writer with the `[id] = true` shape kept, the X on the left of each entry (removeStyle = "icon", LibKa0s v1.44.0) calling it, one repaint per add, a late item load naming its entry, suggestions drawn from the lists and then the loot history newest first, a name several ranks share refused on Enter whether the ranks come from the history or the bags, and each add box's tooltip ending on the same hint its refusal ends with |
...
| `test_itemsetup.lua` | the **`LibKa0s-Item-1.0`** seam (`core/ItemSetup.lua`) — that the four primitives answer what the deleted `Compat` shims answered, that the shims are gone, and that the guessing resolver (`Compat.GetItemInfo`) pointedly **stayed** and still guesses for an uncached item |
...
| `test_surface_parity.lua` | the stub-parity gate, one `Kit.assertSurfaceParity` case per adopted seam (Core, Widgets, Slash, DebugLog, Options, and the v1.55.0 majors: Bus's `Catalog` stub, the Compat member set with its eight unwired members ignored, the Schema stub instance against a live one and the Schema stub library by name; then Item and Pool by name, Pool with its four unused `*Keyed` members ignored, the Lifecycle stand-in against the live latch key for key, and Env and Media as NS name sets derived from their seam files like Core), each degraded arm the same real partial-file-list load. `DebugLog`, `Options`, `Item` and `Pool` name their major and are compared against the live surface `tests/run.lua` registers with `Kit.setSurfaceSource` (the built instance for the first two, the library table for the other two) — the by-name form, which compares `Kit.publicMembers` and so drops LibStub's bookkeeping and every `__`-prefixed library internal. `Core`, `Widgets` and `Slash` stay on the two-table form because none of the three **is** a major's surface: the first two are name sets derived from a seam file of ours, and the third is `settings/Slash.lua`'s own wrapper around the library's dispatcher. Every case is falsifiable by deleting one member from the shipped stub |
| `test_widgets.lua` | the **`LibKa0s-Widgets-1.0`** adoption (`core/WidgetsSetup.lua`), re-pinned to **minor 10** (whose `ReorderList` changes are internal: the drag polls on the library's ghost and takes its line per drag; minor 9 is the release this addon adopts `ReorderList` in) — the seam handing the library its `chevron` and `check` as parameters and deliberately handing it **no** `glyphFont` (with the other half of that decision pinned as source: no option table in this addon sets `glyph`), a **real** row build driven through the library's own `makeMenuRow` by firing a dropdown's actual `OnClick` — never a seeded stand-in row, which is how 553 green library cases sailed over the `FontString:SetText(): Font not set` crash v1.11.0 and v1.11.1 shipped — the highlight and tick painted onto a pooled row, the `isActive` preset row lighting up and its one-click `dd.presets` replacement, a selected character with no option row still counting in the collapsed label, the nine filter-bar instances plus the export modal's picker, the class icon folded into a **label** rather than into an `icon` field the library does not have, `CloseMenu` reached from all three non-click close paths (the window's `OnHide`, `Browser:Hide`, and the export modal's `OnHide`, which had no handler at all before), both hosts' strata proved below `FULLSCREEN`, and the degraded install where the seam answers nil, both surfaces refuse to draw and the addon still comes up |
| `test_launcher.lua` | the **`LibKa0s-Launcher-1.0`** seam (`core/LauncherSetup.lua`) — the minimap button and the broker plugin as **one** object registered twice (launcher-§1): one LDB object of `type = "launcher"` handed straight to LibDBIcon under the addon's **folder** name, wearing the addon's own logo, with `Register` proved idempotent; the **two buttons** (launcher-§2, Launcher minor 4) — left-click opening the settings panel in either state and running no verb, right-click degrading to the panel where there is no `MenuUtil`, and, through `tests/mock_menu.lua` (LibKa0s v1.58.0's own `MenuUtil` stand-in, copied verbatim), the options menu: the brand title and exactly Enabled, Locked, Test mode, Show window in that order, each checkmark read afresh per open, each click reaching the addon's own handler once (the `enable`/`disable`/`test`/`toggle` entries of `NS.COMMANDS`, spied in place, and the *Lock frame* row's `settings.locked` through `Schema:Set`), and while disabled the last three grayed with "enable the addon first" and calling nothing even when force-clicked; the **Minimap button** row driven through `NS.Schema:Set` inverting onto LibDBIcon's own `hide` key and moving the real button (launcher-§3); the seed over `db.global.minimap` still absent; *Reset all settings* re-pointing LibDBIcon at the store's new table; and the **degraded** host with neither broker library, where nothing raises, `Register` reports itself absent and the store is still the truth. The **icon file** is checked as bytes — TGA image type 2, 32 bpp, 128×128 (layout-§4) — because a wrong format there draws nothing and raises nothing, and the TOC's `## IconTexture` is compared against the LDB object's own `icon`. The **status tooltip** (Launcher minor 3) is read through the object's `OnTooltipShow` as exact lines, enabled and disabled — title with the TOC version, `Enabled`, `Locked`, `Test mode`, the record count, `Left-click: Open settings`, `Right-click: Options menu` in both states, nothing twice — with the lock and test mode flipped between shows; the descriptor case re-runs `core/LauncherSetup.lua` into a scratch namespace and pins the retired `onClick` / `leftClickLabel` / `disabledLine` / `slash` absent, the four accessor-and-toggle pairs and `version` present as functions, each toggle routed to its verb's `NS.COMMANDS` handler (or the lock row's `Schema:Set`), and an `onTooltipShow` that draws the count alone |
...
| `test_prose.lua` | the kit’s US-English prose gate (`tests/_kit/`, `localization-§5`), over every authored file `git ls-files` reports — declared by the pair in `tests/run.lua`. It replaced a hand-written `tests/test_prose.lua` that a bare `"test_prose"` entry had been running in its place. The local copy's one waiver (the British-spelled cancellation flag on `NS.After`'s handle, in `core/LifecycleSetup.lua`) was not carried over: that handle is the addon's own wrapper table, which no mock or library reads, so the field is spelled `h.canceled` and the repo ships no `tests/prose_waivers.lua` |
| `test_layout_cap.lua` | the kit’s `layout-§1` cap gate (`tests/_kit/`, kit revision 25), over every authored `.lua` file `git ls-files` reports outside `libs/` and `tests/_kit/` — that none over 1500 lines is missing from the `Files over the 1500-line cap` census under `## Documented deviations` in [ARCHITECTURE.md](ARCHITECTURE.md), that no census row outlives its breach, and that an empty census says so. Declared by the pair in `tests/run.lua` |
| `test_diagnostics.lua` | this addon's half of the **diagnostics report** (`debug-logging-§14`, `modules/Diagnostics.lua`) — the brand on both markers and no section failing, every DX-LH section's lead line in report order, the identity section's schema versions and `account-wide`, a changed setting as `path = value (default)` beside the three always-printed rows, the AH cascade and provider presence, a filter list past 40 ids capped with `(+N more)`, the aggregate history tallies, the 25-record tail with no link escapes, the rejected events `/lh debug events` prints folded in, the stood-down capture line, and what the report must not touch: `GetItemInfo`, `ScanBound`, `C_ChallengeMode` and `Clear` spied, and the registration set, the holds and the debug flag unchanged. Then STD-19's host cases against these sections: a raising section costs one line, an over-cap report ends `truncated` then the end marker, a secret in the stored loot context does not raise, and the context's `detail` table prints as sorted `key=value` pairs rather than an address |
| `test_diagnostics_contract.lua` | the kit’s `debug-logging-§14` dispatcher contract (`tests/_kit/`, kit revision 27), run against this addon’s dispatcher through `Kit.diagnostics` (set in `tests/run.lua`): both forms, the debug word in any case, the branded markers and the counted end, append, the report landing with logging off, both forms while disabled, and `diag`/`dx` running nothing |
...
sits on **v1.36.2**, [`CLAUDE.md`](../CLAUDE.md) names **v1.36.2**, and all four commands above
