...
Thirty-seven suites (see **[test-cases.md](test-cases.md)** for the full per-case inventory and the
authoritative count): thirty-three files of this repo's own under `tests/`, and four the kit ships under
...
| `test_diagnostics_contract.lua` | the kit’s `debug-logging-§14` dispatcher contract (`tests/_kit/`, kit revision 27), run against this addon’s dispatcher through `Kit.diagnostics`. Until the report lands (plan item DR-LH-03) `Kit.diagnostics` is unset and the suite registers one declared skip naming the rule, which is why the README badge reads one short of the total |
