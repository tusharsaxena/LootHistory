...
- **`table`** (the two set-valued rows, `settings.excludedSources` and `settings.auction.capture`) — readable but not writable from the CLI; the parser answers `unknown setting type 'table'`. They are edited from the General ▸ Collection tab and the AH Price page of the settings panel.
