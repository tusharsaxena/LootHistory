...
The dispatcher is the library's (`libs/LibKa0s/Slash.lua:505`), bound onto `NS.Slash` by name at `settings/Slash.lua:208` because ~20 call sites across the schema table, the settings panel and the suite already reach for `NS.Slash:CliList()` and friends:
...
- `Sl:ResetEverything` (`settings/Slash.lua:84`) — the total-reset *composition* behind the panel's "Reset All";
...
`Sl:PrintHelp` prints the header from `Dispatcher:HelpHeader()` (`libs/LibKa0s/Slash.lua:381`) — version, an **em dash**, then the alias clause composed from the descriptor's `slashAliases`:

```
[LH] v1.2.0 — slash commands (|cFFFFFF00/loothistory|r is an alias for |cFFFFFF00/lh|r)
```

then one prefixed row per `NS.COMMANDS` entry, each **indented two spaces** so it sits under that header (`Sl:HelpRows`, `libs/LibKa0s/Slash.lua:375`). A row is a gold command, an em dash with a **single space either side**, and a white description — upper-case hex, because that is the library's:
