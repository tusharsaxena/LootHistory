...
One ordered table drives the entire slash UX: `NS.COMMANDS` in `settings/Schema.lua:637`. Each row is a **positional triple** — `{ name, description, handler }`, the shape `LibKa0s-Slash-1.0` reads — and the same rows dispatch verbs, generate the chat help *and* render the settings landing page, so adding a command is still a one-row append.

The table stays the **host's** and is passed into the library rather than owned by it, and that is the load-bearing decision in this seam rather than squeamishness: the settings landing page renders these same rows (`settings/Panel.lua:838`), so if the slash major owned the table, the options major drawing that page would have to resolve the slash major to read it — a real dependency cycle between two majors at load time. The table crossing between them as plain data is what keeps them independent. Each handler takes the rest of the line verbatim (never a `self`), so the seven verbs that are genuinely this addon's — `show` / `hide` / `toggle` / `config` / `debug` / `test` / `purge` — never leave the host, and adopting the library could not break them.

`/lh` and `/loothistory` are both registered through AceConsole's `RegisterChatCommand` (`Sl:Register`, `settings/Slash.lua:424`) and dispatch to the same `Sl:OnSlash` handler — `/loothistory` is the long-form alias; all help text and docs use the short form.

The dispatcher is the library's (`libs/LibKa0s/Slash.lua:651`), bound onto `NS.Slash` by name at `settings/Slash.lua:394` because ~20 call sites across the schema table, the settings panel and the suite already reach for `NS.Slash:CliList()` and friends:
...
- `Sl:ResetEverything` (`settings/Slash.lua:180`) — the total reset behind the Master controls tab's **Reset all settings** button — options-ui-§12's global reset, and a superset of the `resetall` verb, which never touches history. The two being different acts is a **ratified** divergence from that rule's opening sentence — a register row (`options-ui-§12`, 2026-09-02) in [ARCHITECTURE.md](ARCHITECTURE.md#documented-deviations), reasoned in [schema.md](schema.md#reset-semantics);
- `Sl:CliResetAll` (`settings/Slash.lua:419`) — a **wrapper**, not a re-export, because the `blacklist` / `whitelist` / `currencyBlacklist` id-lists are a structural registry with no schema row, so the library's row walk cannot see them; it clears them by calling their one writer, `NS.Filters:ClearAll` (architecture-§5);
- `Sl.FormatSchemaValue` (`settings/Slash.lua:219`) — passed as the descriptor's `format` hook (Slash **minor 5**), holding **only** the `type = "table"` branch and handing everything else straight back to `lib.FormatValue`;
- `Sl:Register` (`settings/Slash.lua:424`) — the chat-command registration.
...
**Convergence — one command-row formatter, chat and panel.** The settings landing page used to carry its own version of that row: double spaces around the em dash, the dash itself wrapped in white, the description left bare — divergent from the chat help two files away for no reason anyone recorded. Both now render through `lib.FormatRow` (`libs/LibKa0s/Slash.lua:68`) via `Sl:LandingRows` (`settings/Slash.lua:411`), which is the chat form minus the indent — a leading indent reads as a mistake on a panel label, not as structure. User-visible on the landing page: single spaces, no color span on the dash, the description now white. Deliberate; do not "fix" it back (closed issue [LIBKA0S-09](https://github.com/tusharsaxena/LootHistory/issues/24)).
...
- **`enable` / `disable`** — slash-commands-§2's two reserved verbs, and **aliases rather than a second switch**. Each delegates to `Sl:CliSet` with `settings.enabled` and a literal, so the write is the Master controls **Enable Loot History** row's own, through `Schema:Set`, with the row's `onChange` firing the `SettingsChanged` fan-out the Collector listens for. They hold **no state of their own** — no second key, no session flag, no `NS.enabled` local — so the checkbox and the verbs can never show the player two different answers. Delegation rather than a handler is what makes the acknowledgment slash-commands-§5's `set` shape by construction, re-read from the store rather than echoed back; `/lh enable` and `/lh set settings.enabled true` print the identical line, and `tests/test_slash.lua` asserts that.
  ...
  **The dispatcher survives the disabled state**, which is what stops the pair being one-way (§2). Nothing gates *dispatch* on `settings.enabled`: `Sl:Register` runs from `OnInitialize` unconditionally, the chat command is never unregistered, `NS.COMMANDS` is never torn down, and the stored flag's only other reader is `modules/Collector.lua`'s capture gate. A bare `/lh`, `help`, `version`, `list` and `enable` all answer with the addon off.

- **A disabled addon refuses a FEATURE verb** (slash-commands-§2, standard v2.54.0). While `settings.enabled` is `false`, `show` / `hide` / `toggle` / `test` / `purge` — the verbs that draw, preview and destroy — answer on **one** tagged line naming `/lh enable` and **do nothing else**: no partial work, no side effect, no second line. Everything else keeps answering, and that list is §2's verbatim: `help`, `config`, `version`, `enable`, `disable`, `debug`, `perf` and the schema CLI (`get`, `set`, `list`, `reset`, `resetall`). A player must be able to read and repair settings and reach the panel while the addon is off, and `enable` above all. (`perf` is on the list although this addon does not register it — performance-§12, a ratified deviation — because the verb stays reserved here as everywhere.)

  **One gate, at the table.** The wrap is applied to each feature handler as `NS.COMMANDS` is built (`settings/Schema.lua`'s `gateFeatureVerbs`, over the `LIVE_WHILE_DISABLED` set), not pasted into each verb and not placed inside `Sl:OnSlash`. The table is the one seam common to **both** dispatchers this addon has — the library's, and the positional walk the library-less install falls back to — and it is also what a direct `entry[3]("")` call reaches, so there is no route into a verb that skips the gate and a verb declared tomorrow is gated by default. The triple keeps its `name` and `description`, so the help index, the landing page and every count over `NS.COMMANDS` are untouched. The refusal reads through `NS.L.SLASH_DISABLED_VERB` (`locales/enUS.lua`) — the first user-facing string in this addon to route through the locale seam (localization-§1). The switch itself is read through `Schema:Get("settings.enabled")`, never `db.global` directly, so the verbs and the Master controls checkbox can never answer from two places.
...
The `debug` handler (`settings/Schema.lua:668`) drives the debug console independently of the logging flag:
