...
One ordered table drives the entire slash UX: `NS.COMMANDS` in `settings/Schema.lua:512`. Each row is a **positional triple** — `{ name, description, handler }`, the shape `LibKa0s-Slash-1.0` reads — and the same rows dispatch verbs, generate the chat help *and* render the settings landing page, so adding a command is still a one-row append.
...
`/lh` and `/loothistory` are both registered through AceConsole's `RegisterChatCommand` (`Sl:Register`, `settings/Slash.lua:367`) and dispatch to the same `Sl:OnSlash` handler — `/loothistory` is the long-form alias; all help text and docs use the short form.
...
- `Sl:ResetEverything` (`settings/Slash.lua:139`) — the total reset behind the Master controls tab's **Reset all settings** button — options-ui-§12's global reset, and a superset of the `resetall` verb, which never touches history. The two being different acts is a **ratified** divergence from that rule's opening sentence — a register row (`options-ui-§12`, 2026-09-02) in [ARCHITECTURE.md](ARCHITECTURE.md#documented-deviations), reasoned in [schema.md](schema.md#reset-semantics);
- `Sl:CliResetAll` (`settings/Slash.lua:362`) — a **wrapper**, not a re-export, because the `blacklist` / `whitelist` / `currencyBlacklist` id-lists are a structural registry with no schema row, so the library's row walk cannot see them; it clears them by calling their one writer, `NS.Filters:ClearAll` (architecture-§5);
- `Sl.FormatSchemaValue` (`settings/Slash.lua:184`) — passed as the descriptor's `format` hook (Slash **minor 5**), holding **only** the `type = "table"` branch and handing everything else straight back to `lib.FormatValue`;
- `Sl:Register` (`settings/Slash.lua:367`) — the chat-command registration.
...
**Convergence — one command-row formatter, chat and panel.** The settings landing page used to carry its own version of that row: double spaces around the em dash, the dash itself wrapped in white, the description left bare — divergent from the chat help two files away for no reason anyone recorded. Both now render through `lib.FormatRow` (`libs/LibKa0s/Slash.lua:68`) via `Sl:LandingRows` (`settings/Slash.lua:354`), which is the chat form minus the indent — a leading indent reads as a mistake on a panel label, not as structure. User-visible on the landing page: single spaces, no color span on the dash, the description now white. Deliberate; do not "fix" it back (closed issue [LIBKA0S-09](https://github.com/tusharsaxena/LootHistory/issues/24)).
...
The `debug` handler (`settings/Schema.lua:523`) drives the debug console independently of the logging flag:
