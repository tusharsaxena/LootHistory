...
| `Compat.CurrencyQuality(currencyID)` | `C_CurrencyInfo.GetCurrencyInfo` | The currency's `Enum.ItemQuality` tier — colours the Name cell + fills the Quality column for currency rows, and drives the v3→v4 backfill migration; `nil` when uncached/absent. |
