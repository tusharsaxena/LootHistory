...
| `Compat.ItemNameQuality(id)` | `C_Item.GetItemInfo` (bare id) | `(name, quality)` for an item id; `name` is `nil` when the item isn't cached yet. Written to label the blacklist/whitelist rows; **no production caller since the Filters tab adopted LibKa0s IdList (v1.35.0)**, which labels its own rows. |
...
| `Compat.CurrencyName(currencyID)` | `C_CurrencyInfo.GetCurrencyInfo` | Display name for a currency id; `nil` when uncached/absent. Written to label a stored currency-blacklist entry; **no production caller since the Filters tab adopted LibKa0s IdList (v1.35.0)**. |
