...
- `history` is a **dense array** — `Database:Delete`/`PruneOld` rebuild-and-swap rather than leaving holes (`core/Database.lua:718`, `:778`). Each record's field shape is documented in [data-model.md](data-model.md).
...
Six pieces of persisted state live in `db.global` but are written **directly**, bypassing `Schema:Set` — they are runtime/data state, not user settings, and are intentionally not Schema rows (`modules/Browser.lua:100`):

- **`settings.window`** — the browser window geometry `{ point, x, y, w, h }` relative to UIParent. Saved by `SaveWindow` on move/resize (`modules/Browser.lua:106`), restored by `RestoreWindow` on show (`modules/Browser.lua:115`). This is the standalone-windows window position/size persistence.
