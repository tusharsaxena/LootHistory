...
db.global = {
  ...
  settings = {
    ...
    auction = {
      ...
      capture = {},            -- set of "provider:key" tags to gather (Schema row, MultiCheck)
      priority = {},           -- ordered "provider:key" cascade selection list (storage-only, see below)
      priorityDisabled = {},   -- { [tag]=true } — temporarily-disabled priority entries (storage-only, see below)
    },
  },
...
Six pieces of persisted state live in `db.global` but are written **directly**, bypassing `Schema:Set` — they are runtime/data state, not user settings, and are intentionally not Schema rows (`modules/Browser.lua:80`):
...
- **`settings.auction.priority`** — the ordered `"provider:key"` AH-price cascade selection list (`AuctionPrice:Pick` walks it front-to-back; first present key wins). An ordered list has no fixed Schema widget (CheckBox/Dropdown/Slider/MultiCheck) to express reordering, so it is read/written directly via `AuctionPrice:GetPriority`/`MovePriority` (`modules/AuctionPrice.lua`) and managed by the settings panel's own drag/move UI. Its sibling `settings.auction.capture` (which price keys to *gather*, an unordered set) **is** a normal Schema row (`MultiCheck`, `settings/Schema.lua`) — only the ordering half is a carve-out.
- **`settings.auction.priorityDisabled`** — `{ [tag]=true }`, the set of `"provider:key"` priority entries temporarily skipped by `AuctionPrice:Pick` without removing them from the `priority` order (`AuctionPrice:IsPriorityEnabled`/`SetPriorityEnabled`, `modules/AuctionPrice.lua`). Same class of carve-out as `priority` and managed by the same AH Price settings-panel UI (a per-row enable checkbox next to the up/down move arrows) — an ad-hoc boolean keyed by a dynamic tag set, not a fixed Schema widget.

> **Standards note (accepted carve-out).** These six bypass the schema-as-single-source rule (CLAUDE §2, "every user-setting mutation goes through `Schema:Set`"). `window`/`savedView`/`windowScale` were the pre-existing precedent; `blacklist`/`whitelist` and `settings.auction.priority`/`priorityDisabled` extend it for the same reason: a dynamic, unbounded set of arbitrary item ids, or an ordered selection list (and its per-entry enable state), has no fixed schema widget (CheckBox/Dropdown/Slider/MultiCheck) to express it. **Resolution (2026-07-17): ratified as a legitimate carve-out** for `blacklist`/`whitelist`, same class as `window`/`savedView`, managed by `NS.Filters` writing `NS.db.global` directly. `settings.auction.priority` follows the same precedent (Rev-2 R5, 2026-07-19): an ordered list is not one of the four schema widget types, so it is managed directly by `modules/AuctionPrice.lua` + the settings panel. `settings.auction.priorityDisabled` (Rev-3 R3, 2026-07-19) rides along as its direct sibling — a per-tag enable flag for the same UI, same file, same non-Schema-widget reasoning. The Ka0s Standard's own definition was left unchanged; if a future addon wants either pattern first-class, amend the standard's schema section then.
