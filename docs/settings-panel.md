...
Every setting mutation — panel widget and `/lh set` alike — routes through `NS.Schema:Set(path, value)` (`Schema.lua:363`). The library never learns a path or a database: the descriptor hands it `get` / `set` / `applyDefault` closures over this seam (`OptionsSetup.lua:159`), which is what guarantees a panel click takes exactly the path a slash command does.
