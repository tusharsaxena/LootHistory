...
Ten rows ship today (`Schema.lua:11`): `settings.enabled`, `minimap.hide`, `state.debugConsole`, `settings.windowScale` (Master Controls); `settings.qualityThreshold`, `settings.excludeQuestItems`, `settings.retentionDays`, `settings.excludedSources` (Data Collection); `settings.auction.enabled`, `settings.auction.capture` (AH Price — the latter carries `panelSkip`, driven by the price table rather than a rendered widget).
