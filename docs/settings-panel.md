...
* **Blacklist** — items you never want tracked. Add an item by its id, by shift-clicking an item link into the box, or by typing the name of an item your client has seen, then press Enter or **Add**. If nothing matches, nothing is added and the line under the box says why. This is point-in-time: once an id is blacklisted, future loots of it are skipped and never recorded, but rows you've *already* recorded are left exactly where they are — editing the list doesn't touch stored history. Delete a row manually if you want it gone.
...
* **Currencies** — currencies you never want recorded, on the same point-in-time terms. Add one by its id or by shift-clicking a currency link into the box. Names don't work here: the game has no way to look a currency up by name.
...
* **The add box** takes a bare id, a shift-clicked link, or — on the two item lists (`kind = "item"`) — the name of an item the client has seen, resolved through `O.ResolveId`. The Currencies list is `kind = "currency"`: the client has no currency name lookup, so it takes an id or a currency link only, and a name is refused with *"Currencies are added by id or link."* An input that resolves to nothing adds nothing, keeps the text, and says why on the status line under the box.
