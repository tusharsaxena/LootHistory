...
`BrowserTable:ToggleTestMode` (`BrowserTable.lua:406`) publishes a synthetic dataset to `NS.State.testRecords` and opens the window. Because `Database:ActiveHistory` (`Database.lua:60`) returns `NS.State.testRecords` when present, the fake data flows through the *same* read paths (`Query` / `Stats` / `CurrentRecords`) — so it drives **both** the History table and the Insights charts at once, while write paths (Add / prune) always target the real history and never see the override. `OnDatasetChanged` (`Browser.lua:627`) rebuilds the filter dropdowns from the new dataset, resets to the stock view + all players in test mode (test characters differ from the current one), and toggles the red "TEST MODE" badge beside the title.
