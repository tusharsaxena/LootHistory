...
`B:BuildHistory` (`Browser.lua:683`) lays out a **two-row** filter bar in the History pane, ordered to mirror the table's columns:

- **Row 1** — Group-by dropdown · item-name search box · right-aligned `Save` · `Reset` · `Clear` buttons.
- **Row 2** — the column filters left→right in table order: Date · Quality · Type · Source · Zone · Character, plus a right-aligned player-scope toggle.
