...
The **date dropdown** is single-select (Today / Last 7 days / Last 30 days / All); it writes `activeFilter.from` from `NS.Util.RangeFrom(v)` (`Browser.lua:767`). The **search box** writes `activeFilter.text` on every keystroke (case-insensitive substring on item name). The **data-driven dropdowns (quality/source/type/subtype/char/zone)** are rebuilt from the current dataset by `RefreshFilterOptions`, each distinct-valued and prefixed with an "All" sentinel. **Quality is data-driven** (`qualityOptions`) rather than a fixed 1–5 list, sorted in quality order and tinted per quality — so any quality the history actually contains (Heirloom, Poor, Artifact) shows up, matching the Insights "Quality distribution", and absent qualities don't clutter the menu.
