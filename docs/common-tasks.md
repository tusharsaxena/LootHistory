...
- **Every setting mutation routes through `Schema:Set(path, value)`** (`settings/Schema.lua:664`),
...
- The flag is independent of the console window's visibility. `/lh debug` toggles the window only;
  ...
  `settings/Schema.lua:791`); the header's `Debug: ON`/`OFF` control flips the same flag
