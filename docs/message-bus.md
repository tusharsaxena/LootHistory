...
- `Database:PruneOld()` — retention rebuild-and-swap (also invoked when the player accepts the `retentionDays` row's prune confirm, so a retention change surfaces as `HistoryChanged`, not `SettingsChanged`).
