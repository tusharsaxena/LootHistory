...
- **Browser** (`modules/Browser.lua:1189`) calls `OnSettingsChanged()` to reflect the change in the open window.
...
- Browser — `B.__ev = NS.NewBusTarget()` (`modules/Browser.lua:1276`).
- Analytics — `self.__ev = NS.NewBusTarget()` (`modules/Analytics.lua:651`).
- Panel — `local ev = NS.NewBusTarget()`, **twice**: the History tab's storage readout (`settings/Panel.lua:154`) and the Filters tab's id-lists (`settings/Panel.lua:442`), each on its own target.
