...
- Panel — `local ev = NS.NewBusTarget()`, **twice**: the History tab's storage readout (`settings/Panel.lua:155`) and the Filters tab's id-lists (`settings/Panel.lua:436`), each on its own target.
