...
![Tests](https://img.shields.io/badge/Tests-166%2F166_passing-green)
...
|---------|------|------------|
| 1.1.0 | 2026-07-12 | Maintenance & standards-compliance release.<br>**Changed:** the settings panel's scrollbar now stays visible (inert when the page fits), so the layout no longer shifts between sections.<br>**Fixed:** the Reset All / Purge buttons are no longer clipped at the panel's right edge.<br>**Internals:** added a schema-migration seam (`RunMigrations`) for future database upgrades; brought the addon into full compliance with the Ka0s WoW Addon Standard (TOC metadata, packaging, `media/logos/`); Retail-only compatibility cleanup; headless test suite expanded (118 → 124).<br>**Docs:** restructured to the standard's layout (root README + stub CLAUDE; ARCHITECTURE and the agent brief under `docs/`); added the standard badge; migrated the backlog from `TODO.md` to the GitHub issue tracker. |
